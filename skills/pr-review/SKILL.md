---
name: pr-review
description: Use when reviewing a PR/MR diff for security, logic, performance, cross-file impact, test coverage, or spec compliance findings. NOT for writing PR descriptions, design reviews requiring business judgment, implementation work, release notes, or deep CVE/supply-chain audits. This reviews CODE DIFFS — not requirement docs (that's prd-dev-readiness) or development plans (that's plan-review). For a comprehensive multi-angle review that also covers dedicated test effectiveness, quality/simplification, and deep security in one pass — or when unsure which review tool fits — use review-router, which orchestrates this skill alongside the others.
---
